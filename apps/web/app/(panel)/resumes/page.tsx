...
export default function ResumesPage() {
  ...
  const requestedResumeId = searchParams.get("openResume") || "";
  ...
  useEffect(() => {
    ...
    void getActiveProfileId()
      ...
      .then(async ([resumes, knowledge, jobs]) => {
        ...
        if (!active) return;
        const requestedResume = requestedResumeId
          ? restoredResumes.find((item) => item.id === requestedResumeId)
          : undefined;
        const resume = requestedResume || restoredResumes[0];
        ...
        <div className="flex min-w-0 flex-1 gap-1.5 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {templateCategories.map((category) => {
            ...
            return (
              ...
              >
                <span className="flex items-center px-[11px] py-1">
                  <span
                    aria-hidden="true"
                    className={cn(
                      "grid shrink-0 overflow-hidden transition-[width,margin] duration-300 ease-in-out motion-reduce:transition-none",
                      active ? "ml-[7px] w-3" : "ml-0 w-0",
                    )}
                  >
                    <Check className="min-w-3" size={12} strokeWidth={3} />
                  </span>
                  <span>{category}</span>
                </span>
                <span
