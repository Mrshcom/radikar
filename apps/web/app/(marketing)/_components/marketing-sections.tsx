...
function ProductStage() {
  ...
  return (
    <div id="product" className={`${shell} relative mt-16 scroll-mt-24`}>
      <h2 className="sr-only">نمونه تحلیل تطابق رزومه با فرصت شغلی در رادیکار</h2>
      <div className="relative overflow-hidden rounded-[30px] border border-[#55ac86]/50 bg-[linear-gradient(145deg,#1a624b_0%,#0e4032_48%,#08291f_100%)] p-2 shadow-[0_42px_110px_rgba(10,70,50,.25)] sm:p-3">
        ...
        <div className="relative flex h-10 items-center justify-between px-4">
          ...
          <span className="text-[8px] font-bold text-white/75">app.radikar.ir</span>
          <span />
        </div>
        ...
        <div className="relative grid min-h-[560px] overflow-hidden rounded-[22px] bg-[#f5f7f3] lg:grid-cols-[190px_1fr]">
          <aside className="hidden border-l border-black/6 bg-white p-4 lg:flex lg:flex-col">
            <div className="mb-7 flex items-center gap-2">
              ...
              <div><strong className="block text-[11px]">رادیکار</strong><span className="text-[7px] text-[#596a64]">پنل مسیر شغلی</span></div>
            </div>
            {navigation.map(({ label, icon: Icon }, index) => (
              <div key={label} className={`mb-1.5 flex items-center gap-2 rounded-xl px-3 py-2.5 text-[9px] font-bold ${index === 3 ? "bg-[#e4f8ed] text-[#087b4f]" : "text-[#52645e]"}`}>
                <Icon size={13} /> {label}
              </div>
            ))}
          ...
          <div className="p-4 sm:p-6">
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div>
                <div className="flex items-center gap-2"><span className="text-[9px] font-bold text-[#52645e]">تحلیل فرصت شغلی</span><span className="rounded-full bg-[#e2f5ea] px-2 py-1 text-[7px] font-black text-[#0b7b54]">تحلیل کامل</span></div>
                <h3 className="mt-1 text-lg font-black text-[#18211f]">طراح محصول ارشد · دورکاری</h3>
                <div className="mt-2 flex flex-wrap gap-3 text-[8px] text-[#52645e]"><span className="flex items-center gap-1"><BriefcaseBusiness size={11}/> راهکارنو</span><span className="flex items-center gap-1"><MapPin size={11}/> تهران / دورکاری</span><span>تمام‌وقت</span><span>منتشرشده ۲ روز پیش</span></div>
              </div>
            ...
            <div className="mt-5 grid gap-4 xl:grid-cols-[1fr_220px]">
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="col-span-full rounded-2xl border border-black/6 bg-white p-4">
                  <div className="flex items-center justify-between"><strong className="text-[9px]">جزئیات امتیاز تطابق</strong><span className="text-[8px] text-[#52645e]">براساس ۲۴ معیار آگهی</span></div>
                ...
                <div className="col-span-full rounded-2xl border border-black/6 bg-white p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-[#e3f5eb] text-[#16805b]"><FileText size={15}/></span><div><strong className="block text-[9px]">رزومه اختصاصی برای همین فرصت</strong><span className="mt-1 block text-[7px] text-[#52645e]">پایه: رزومه محصول ۱۴۰۵ · بدون تغییر اطلاعات واقعی</span></div></div><span className="rounded-full bg-[#e3f5eb] px-3 py-1.5 text-[8px] font-black text-[#087b5b]">آماده ساخت</span></div>
                ...
                <div className="mt-3 flex items-center justify-between"><span className="text-[9px] text-[#596a64]">نسخه جدید کنار رزومه اصلی ذخیره می‌شود.</span><span className="rounded-xl bg-[#176b4e] px-4 py-2.5 text-[9px] font-black text-white">ساخت نسخه اختصاصی</span></div>
                </div>
              </div>
              ...
              <div className="relative flex min-h-[390px] flex-col overflow-hidden rounded-2xl border border-[#57a984]/35 bg-[radial-gradient(circle_at_50%_15%,#236e53_0%,#124936_45%,#0b3529_100%)] p-5 text-center text-white shadow-[0_18px_45px_rgba(10,66,47,.22)]">
                ...
                <span className="relative text-[9px] text-white/75">امتیاز تطابق کلی</span>
                ...
                <span className="relative mt-1 text-[8px] text-white/75">شانس بالا برای عبور از غربال اولیه</span>
                <div className="relative mt-5 grid gap-2 border-t border-white/10 pt-4 text-right text-[8px]">
                  <div className="flex justify-between"><span className="text-white/75">معیارهای منطبق</span><strong>۲۱ از ۲۴</strong></div>
                  <div className="flex justify-between"><span className="text-white/75">مهارت کلیدی مشترک</span><strong>۱۲ مورد</strong></div>
                  <div className="flex justify-between"><span className="text-white/75">شکاف قابل بهبود</span><strong className="text-[#f0cf7b]">۳ مورد</strong></div>
                </div>
                <div className="relative mt-auto rounded-xl border border-white/10 bg-white/[.07] p-3 text-right"><span className="text-[7px] text-white/75">پیشنهاد رادیکار</span><p className="mb-0 mt-1 text-[8px] leading-5 text-white/75">با یک نسخه هدفمند از رزومه، امتیاز کلیدواژه‌ها تا ۹۰٪ بهتر می‌شود.</p></div>
              </div>
            </div>
          </div>
        </div>
...
function FeaturePreview({ index }: { index: number }) {
      ...
      <div className={previewClassName}>
        ...
        <div className="mt-3 flex items-center justify-between gap-4">
          <div className="min-w-0">
            ...
            <span className="mt-1 flex items-center gap-1 text-[8px] text-[#52645e]"><MapPin size={10} /> راهکارنو · تهران / دورکاری</span>
          </div>
      ...
      <div className={previewClassName}>
        ...
        <div className="mt-3 flex items-start gap-3 rounded-xl bg-[#f4f8f5] p-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <span className="mt-1 block text-[7px] text-[#52645e]">۹۱٪ تطابق با شرح شغل</span>
          </div>
        </div>
  ...
  return (
    <div className={previewClassName}>
      ...
      <div className="mt-3 flex items-center justify-between rounded-xl bg-[#f2f7f4] px-3 py-2"><div><strong className="block text-[8px] text-[#18382c]">طراح محصول ارشد</strong><span className="mt-0.5 block text-[7px] text-[#52645e]">راهکارنو</span></div><span className="rounded-full bg-[#fff1cf] px-2 py-1 text-[7px] font-black text-[#8a6718]">مصاحبه فردا · ۱۰:۳۰</span></div>
    </div>
  );
}
            ...
            >
              ...
              <div className="relative z-10">
                <div className="text-right">
                  ...
                  <p className={`mt-2 text-xs ${plan.featured ? "text-white/75" : "text-[#596a64]"}`}>{plan.description}</p>
                </div>
                <div className="mt-9 flex items-end gap-2 border-b border-black/8 pb-8">
                  ...
                  {plan.price !== "رایگان" && (
                    <small className={`mb-1 text-[11px] ${plan.featured ? "text-white/75" : "text-[#596a64]"}`}>
                ...
                <ul className={`mt-8 flex-1 space-y-5 p-0 text-[12px] ${plan.featured ? "text-white/80" : "text-[#53635e]"}`}>
                  {plan.items.map(([item, included]) => (
                    <li key={item} className={`flex items-center gap-3 ${included ? "" : plan.featured ? "text-white/30" : "text-[#6a7873]"}`}>
...
function FaqItem({ question, answer }: { question: string; answer: string }) {
  return <details className="group rounded-2xl border border-black/7 bg-[#fbfcf8] px-5 py-4">
    <summary className="flex cursor-pointer list-none items-center justify-between text-right text-xs font-black marker:content-none">
      {question}<ChevronDown size={16} aria-hidden="true" className="shrink-0 transition duration-300 ease-out group-open:rotate-180" />
    </summary>
    <p className="mt-3 border-t border-black/5 pt-3 text-[11px] leading-7 text-[#596a64]">{answer}</p>
  </details>;
}

export function FaqSection(){return <section id="faq" className="bg-white py-24"><div className={`${shell} grid gap-12 lg:grid-cols-[.8fr_1.2fr]`}><SectionIntro eyebrow="سؤال‌های پرتکرار" title="پاسخ پرسش‌های مهم درباره رادیکار" text="تعریف روشن قابلیت‌ها، نحوه تطبیق رزومه و چگونگی نگهداری اطلاعات را اینجا بخوان."/><div className="space-y-3">{marketingFaqs.map(({question,answer})=><FaqItem key={question} question={question} answer={answer}/>)}</div></div></section>}
...
export function MarketingFooter() {
  const footerLinks = [
    { title: "محصول", links: [["رزومه‌ساز", "/resume-builder"], ["تطبیق رزومه و شغل", "/resume-job-match"], ["مدیریت اپلای", "/application-tracker"]] },
    { title: "رادیکار", links: [["راهنماها", "/guides"], ["درباره ما", "/about"], ["ارتباط با ما", "/contact"]] },
  ];
      ...
      <div className={`${shell} relative py-14 sm:py-16`}>
        <div className="grid gap-12 border-b border-white/10 pb-12 lg:grid-cols-[1.35fr_.65fr_.65fr]">
          <div className="max-w-md">
            ...
            <p className="mt-6 text-[13px] leading-7 text-white/75">فرصت‌های شغلی متناسب را پیدا کن، میزان تطابقت را بسنج و برای هر موقعیت یک رزومه هدفمند و آماده ارسال داشته باش.</p>
...
function SectionIntro({eyebrow,title,text,dark=false}:{eyebrow:string;title:string;text:string;dark?:boolean}){return <div className="max-w-2xl"><span className={`text-[10px] font-black ${dark?"text-[#b8f4d3]":"text-[#0b704c]"}`}>● {eyebrow}</span><h2 className={`mt-4 text-3xl font-black leading-[1.5] sm:text-4xl ${dark?"text-white":"text-[#17201e]"}`}>{title}</h2><p className={`mt-4 max-w-xl text-xs leading-8 ${dark?"text-white/75":"text-[#52645e]"}`}>{text}</p></div>}
