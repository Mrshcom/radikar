...
export function JobCard({ job, saved = false, onSave }: { job: Job; saved?: boolean; onSave?: () => void }) {
  const logoLetter = Array.from(job.company.trim())[0] || "م";

  return <article className="job-card">
    <div className="job-card-top"><div className={`brand-logo ${job.tone}`} aria-hidden="true">{logoLetter}</div><div className="job-top-actions"><span className="match-pill"><Target size={13} /> تطابق {job.match}٪</span>{onSave && <button className={`save-job ${saved ? "saved" : ""}`} onClick={onSave} aria-label={saved ? "حذف از ذخیره‌ها" : "ذخیره فرصت"}>{saved ? <BookmarkCheck size={16} /> : <Bookmark size={16} />}</button>}</div></div>
