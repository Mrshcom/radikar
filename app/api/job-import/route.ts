...
export async function POST(request: NextRequest) {
  ...
  if (
    ...
    return NextResponse.json(
      { error: "فعلاً فقط لینک job boardهای معتبر پشتیبانی می‌شود." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return NextResponse.json(
        { error: "این لینک redirect دارد؛ لینک نهایی آگهی را وارد کن." },
        { status: 400 },
      );
    }
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: "متن آگهی از این لینک قابل دریافت نبود." },
        { status: 502 },
      );
    }
    ...
    if (
      ...
      return NextResponse.json(
        { error: "این لینک صفحه HTML قابل خواندن برنگرداند." },
        { status: 415 },
      );
    }
    ...
    if (text.length < 80) {
      return NextResponse.json(
        { error: "متن کافی برای پردازش از این آگهی پیدا نشد." },
        { status: 422 },
      );
    }
    ...
    return NextResponse.json(
      { error: "خواندن لینک آگهی با خطا روبه‌رو شد." },
      { status: 502 },
    );
  }
}
