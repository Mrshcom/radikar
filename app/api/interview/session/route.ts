...
export async function POST(request: NextRequest) {
  ...
  if (!hasResumeContent(body.resume)) {
    return NextResponse.json(
      { error: "برای ساخت جلسه مصاحبه ابتدا رزومه را تکمیل کن." },
      { status: 422 },
    );
  }
  const resume = { ...emptyResumeData, ...body.resume };
  try {
    const result = await chatJson<Partial<InterviewSession>>(
      getAnalyzeConfig(),
      [
        {
          role: "system",
    ...
    if (
      ...
      return NextResponse.json(
        { error: "مدل جلسه مصاحبه کامل تولید نکرد." },
        { status: 502 },
      );
    }
    return NextResponse.json({
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "مدل مصاحبه پاسخ نداد.",
      },
      { status: 502 },
    );
  }
